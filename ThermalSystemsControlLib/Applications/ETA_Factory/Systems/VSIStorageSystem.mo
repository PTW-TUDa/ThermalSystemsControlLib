...
model VSIStorageSystem
  extends ThermalSystemsControlLib.BaseClasses.AutomationBaseClasses.SystemContinuous_LocalControlMode(
                                                                                              systemFlowControl(nComponents=2));
  ...
  Components.Pumps.Pump PU305(pumpType=11) annotation (Placement(transformation(extent={{10,-10},{-10,10}},
        rotation=180,
        origin={70,90})));
  Components.Valves.ThreeWayValve SV306(redeclare Records.Belimo_R2020_S2_ThreeWay deviceData) annotation (Placement(transformation(extent={{80,-70},{100,-50}})));
  Components.Pumps.Pump PU306(pumpType=11) annotation (Placement(transformation(extent={{80,-100},{100,-80}})));
  Components.Valves.ThreeWayValve SV305(redeclare Records.Belimo_R2020_S2_ThreeWay deviceData) annotation (Placement(transformation(extent={{80,70},{100,50}})));
  Components.Valves.TwoWayValve SV307(redeclare Records.Belimo_R2020_S2 deviceData) annotation (Placement(transformation(extent={{80,-40},{100,-20}})));
  ...
  Components.BufferStorage.Interfaces.BufferStorageState localState annotation (Placement(transformation(extent={{-10,-120},{10,-100}})));
  ...
  Components.Pipes.PhysicalModels.Pipe pipe2(
    ...
    n_Bending=0) annotation (Placement(transformation(
        ...
        rotation=180,
        origin={134,20})));
  Modelica.Blocks.Math.IntegerToBoolean integerToBoolean "0 = unloading, 1 = loading" annotation (Placement(transformation(extent={{-40,-80},{-20,-60}})));
equation
  ...
  connect(WMZ305.fHeatFlowRate, SV307.fThermalPowerExternal) annotation (Line(points={{79,30},{94,30},{94,-30},{90,-30},{90,-42}}, color={0,0,127}));
  ...
  connect(VSIStorage.localState, localState) annotation (Line(points={{90,-11},{0,-11},{0,-110}}, color={0,0,0}));
  ...
  connect(PU305.fThermalPowerExternal, WMZ305.fHeatFlowRate) annotation (Line(points={{70,102},{70,92},{79,92},{79,30}}, color={0,0,127}));
  connect(PU306.fThermalPowerExternal, WMZ305.fHeatFlowRate) annotation (Line(points={{90,-102},{90,-100},{79,-100},{79,30}}, color={0,0,127}));
  ...
  connect(VSIStorage.port_a, pipe2.port_a) annotation (Line(points={{100,10},{124,10}}, color={0,127,255}));
  connect(WMZ305.port_a, pipe2.port_b) annotation (Line(points={{100,20},{112,20},{112,30},{124,30}}, color={0,127,255}));
  connect(selectLocalControlMode.nControlMode, integerToBoolean.u) annotation (Line(points={{-59,-70},{-42,-70}}, color={255,127,0}));
  connect(integerToBoolean.y, SV306.bSetStatusOnAutomatic) annotation (Line(points={{-19,-70},{-10,-70},{-10,-51},{78,-51}}, color={255,0,255}));
  connect(not1.u, SV306.bSetStatusOnAutomatic) annotation (Line(points={{38,50},{28,50},{28,-51},{78,-51}}, color={255,0,255}));
  connect(and1.u2, SV306.bSetStatusOnAutomatic) annotation (Line(points={{-22,22},{-22,-50},{-10,-50},{-10,-51},{78,-51}}, color={255,0,255}));
