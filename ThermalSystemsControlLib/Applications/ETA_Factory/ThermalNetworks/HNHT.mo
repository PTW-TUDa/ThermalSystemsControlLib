...
model HNHT
  ...
  Components.Pipes.PhysicalModels.Pipe pipe14(
    use_HeatLoss=false,
    length=12,
    diameter=0.05,
    n_Bending=6) annotation (Placement(transformation(
        ...
        rotation=270,
        origin={-70,30})));
  Components.Pipes.PhysicalModels.Pipe pipe17(
    use_HeatLoss=false,
    length=12,
    diameter=0.05,
    n_Bending=6) annotation (Placement(transformation(
  ...
  Components.Pipes.PhysicalModels.Pipe pipe15(
    ...
    n_Bending=3) annotation (Placement(transformation(
        ...
        rotation=90,
        origin={62,70})));
  Modelica.Blocks.Math.Gain gain(k=1000) annotation (Placement(transformation(extent={{104,4},{110,10}})));
...
equation
  ...
  VSIStorageSystem.fSetPointAutomatic = 50;
  ...
  connect(CondensingBoilerSystem.port_a, pipe17.port_b) annotation (Line(points={{-80,-10},{-80,-20}}, color={0,127,255}));
  connect(CondensingBoilerSystem.port_b, pipe14.port_a) annotation (Line(points={{-80,10},{-80,20}}, color={0,127,255}));
  connect(pipe14.port_b, pipe2.port_a) annotation (Line(points={{-60,20},{-40,20}}, color={0,127,255}));
  connect(pipe17.port_a, pipe3.port_b) annotation (Line(points={{-60,-20},{-40,-20}}, color={0,127,255}));
  connect(VSIStorageSystem.port_a2, pipe15.port_b) annotation (Line(points={{40,72},{40,80},{52,80}}, color={0,127,255}));
  connect(pipe15.port_a, BufferStorage.port_b) annotation (Line(points={{72,80},{74,80},{74,40},{40,40},{40,10}}, color={0,127,255}));
  connect(VSIStorageSystem.port_b2, BufferStorage.port_a) annotation (Line(points={{40,52},{48,52},{48,-10},{40,-10}}, color={0,127,255}));
  ...
  connect(port_a_HNLT_Producer, BufferStorage.port_b) annotation (Line(points={{100,100},{44,100},{44,20},{40,20},{40,10}}, color={0,127,255}));
  connect(combiTimeTable.y[1], gain.u) annotation (Line(points={{96.5,7},{100.25,7},{100.25,7},{103.4,7}}, color={0,0,127}));
  connect(gain.y, CentralMachineHeatingSystem.fHeatFlowRate) annotation (Line(points={{110.3,7},{114.15,7},{114.15,7},{118,7}}, color={0,0,127}));
  connect(lessThreshold.u, gain.u) annotation (Line(points={{103.4,-9},{100,-9},{100,7},{103.4,7}}, color={0,0,127}));
  connect(multiSum_gas.u[1], CondensingBoilerSystem.P_gas) annotation (Line(points={{-64,87.2},{-72,87.2},{-72,1},{-79,1}}, color={0,0,127}));
  connect(CHP2System.P_gas, multiSum_gas.u[2]) annotation (Line(points={{-39,-1},{-36,-1},{-36,90},{-64,90}}, color={0,0,127}));
  connect(CHP1System.P_gas, multiSum_gas.u[3]) annotation (Line(points={{1,-1},{10,-1},{10,92.8},{-64,92.8}}, color={0,0,127}));
  connect(CHP2System.P_el, multiSum_el.u[1]) annotation (Line(points={{-39,1},{-39,67.9},{-64,67.9}}, color={0,0,127}));
  connect(multiSum_el.u[2], CHP1System.P_el) annotation (Line(points={{-64,72.1},{1,72.1},{1,1}}, color={0,0,127}));
