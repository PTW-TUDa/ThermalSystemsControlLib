...
model HeatMeter
  ...
  Modelica.Blocks.Interfaces.RealOutput fHeatFlowRate "Temperature of the passing fluid" annotation (Placement(transformation(
        ...
        rotation=180,
        origin={-110,0})));
  Interfaces.heatMeterState_FMUConnector sensorState annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={50,30})));
equation
  ...
  connect(sensorState.sensorState, heatMeter_Physical.sensorState) annotation (Line(points={{50,19},{50,11},{61,11}},
                                                                                                                    color={0,0,0}));
