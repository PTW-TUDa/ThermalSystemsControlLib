...
model DryCooler_Physical
  ...
  HeatExchanger.PhysicalModels.HeatExchanger          heatExchanger_Physical(
  ...
  Pumps.PhysicalModels.IdealPump_Physical idealPump_Physical(redeclare package Medium = Modelica.Media.Air.SimpleAir, deviceData=deviceData.deviceData_Ventilation) annotation (Placement(transformation(
        extent={{-10,10},{10,-10}},
        rotation=270,
        origin={30,30})));
equation
  ...
  connect(boundary1.ports[1], heatExchanger_Physical.port_b2) annotation (Line(points={{-82,20},{-20,20},{-20,14},{-8,14}},      color={0,127,255}));
  connect(heatExchanger_Physical.port_a1, port_a) annotation (Line(points={{-8,6},{-20,6},{-20,0},{-100,0}},          color={0,127,255}));
  connect(T_air, boundary.T_in) annotation (Line(points={{-120,50},{82,50},{82,16}}, color={0,0,127}));
  connect(fSetPoint, idealPump_Physical.fSetPoint) annotation (Line(points={{0,120},{0,30},{18,30}},         color={0,0,127}));
  connect(idealPump_Physical.fOperatingPoint, fOperatingPoint) annotation (Line(points={{26,19},{26,40},{110,40}},         color={0,0,127}));
  ...
  connect(idealPump_Physical.port_b, heatExchanger_Physical.port_a2) annotation (Line(points={{20,20},{20,14},{12,14}}, color={0,127,255}));
